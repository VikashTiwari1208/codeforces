    ...
    fast;
       test 
       {
        ...
        else
        {
            vvi dp1(n,vi(m,-1e7));
            vvi dp2(n,vi(m,1e7));
            dp1[0][0]=dp2[0][0]=v[0][0];
            for(int i=1;i<m;i++)
            {
              dp1[0][i]=dp1[0][i-1]+v[0][i];
              dp2[0][i]=dp2[0][i-1]+v[0][i];
            }
            for(int i=1;i<n;i++)
            {
            ...
            for(int i=1;i<n;i++)
            {
                for(int j=1;j<m;j++)
                {
                    dp1[i][j]=max(dp1[i][j-1]+v[i][j],dp1[i-1][j]+v[i][j]);
                    dp2[i][j]=min(dp2[i][j-1]+v[i][j],dp2[i-1][j]+v[i][j]);
                }
            }
             if(0>=dp2[n-1][m-1]&&0<=dp1[n-1][m-1])
             {
                cout<<"YES";
             }
             else
