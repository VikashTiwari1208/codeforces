...
int32_t main()
{
    ...
    while(side>0)
    {
        for(int i=start.first;i<=start.first+side-1;i++)
        {
            v[i][start.se]=col;
        }
        for(int i=start.second;i<=start.second+side-1;i++)
        {
            v[start.fi][i]=col;
        }
        for(int i=start.first;i<=start.fi+side-1;i++)
        {
            v[i][start.second+side-1]=col;
        }
        for(int i=start.second;i<=start.second+side-1;i++)
        {
            v[start.first+side-1][i]=col;
        }
      start.first++;
      start.second++;  
     side-=2;
