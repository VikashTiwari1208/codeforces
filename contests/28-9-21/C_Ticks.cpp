...
// ye dekhlena
bool isvalid(int n,int m,int x,int y,vector<vector<char>>&arr) {
         ...
         for(int i=0;i<n;i++)
         {
             for(int j=0;j<m;j++)
             {
                if(arr[i][j]=='*')
                {
                    int idx=1;
                    int len=0;
                   while(isvalid(n,m,i-idx,j+idx,arr)&&isvalid(n,m,i-idx,j-idx,arr))
                   {
                       len++;idx++;
                   }
                //    if(i==1&&j==1)
                ...
                //    }
                   if(len>=k)
                   {
                      ...
                      while(len>0)
                     {
                       if(isvalid(n,m,i-idx,j+idx,arr)&&(isvalid(n,m,i-idx,j-idx,arr)))
                       {
                          vis[i-idx][j-idx]=1;
                          vis[i-idx][j+idx]=1;
                       }
