       ...
       test 
       {
         ...
         cin>>n;
          ...
          for(int i=0;i<n-1;i++)
          {
              int a=max(v[i],v[i+1]);
              int b=min(v[i],v[i+1]);
              int z=(int)ceil((double)a/(double)b);
             // cout<<z<<endl;;
              if(z>2)
              {
                 int maxi=max(v[i],v[i+1]);
                 int mini=min(v[i],v[i+1]);
                 int temp=ceil((double)maxi/(double)2);
                 int z1=ceil((double)temp/(double)mini);
                 cnt++;
               //  cout<<maxi<<" "<<mini<<" "<<temp<<" "<<z1<<" "<<endl;;
                 while(z1>2)
                 {
                   temp=ceil((double)temp/(double)2);
                   z1=ceil((double)temp/(double)mini);
                   cnt++;
                    // cout<<temp<<" "<<z1<<" "<<cnt<<endl;
                 }
              }
          }
          cout<<cnt<<endl;
