       ...
       test 
       {
        ...
        cin>>n>>m;
         ...
         for(int i=0;i<n-1;i++)
         {
            for(int j=0;j<m-1;j++)
            {
                int sum=vv[i][j]+vv[i+1][j+1]+vv[i+1][j]+vv[i][j+1];
                if(sum==0)
                {
                    continue;
                }
              int way1=max((int)1,vv[i][j]+vv[i+1][j]+vv[i+1][j+1]);
              int way2=1e7,way3=1e7,way4=1e7;
              way2=max((int)1,vv[i][j]+vv[i][j+1]+vv[i+1][j+1]);
              way3=max((int)1,vv[i+1][j]+vv[i+1][j+1]+vv[i][j+1]);
              way4=max((int)1,vv[i+1][j]+vv[i][j+1]+vv[i][j]);
              miniside=min({miniside,way1,way2,way3,way4});
            }
         }
         //cout<<miniside<<endl;
        if(cnto==0)
