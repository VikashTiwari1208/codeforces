...
int32_t main()
{
    fast;
         ...
         for(int i=n-1;i>=0;i--)
         {
             ...
             {
                 if(mp.find(queries[i].first)==mp.end())
                 {
                     //cout<<queries[i].first<<" ";
                     ans.pb(queries[i].first);
                     mp[queries[i].first]=queries[i].first;
                   //   cout<<i<<" "<<mp[queries[i].first]<<" "<<mp[queries[i].second]<<endl;
                 }
                 else
                 {
                      //cout<<mp[queries[i].first]<<" ";
                     ans.pb(mp[queries[i].first]);
                      //cout<<i<<" "<<mp[queries[i].first]<<" "<<mp[queries[i].second]<<endl;
                 }
             }
             else
             {
                 if(mp.find(queries[i].second)==mp.end())
                 {
                     mp[queries[i].first]=queries[i].second;
                     mp[queries[i].second]=queries[i].second;
                    // cout<<i<<" "<<mp[queries[i].first]<<" "<<mp[queries[i].second]<<endl;
                 }
                 else
                 {
                     mp[queries[i].first]=mp[queries[i].second];
                 }
             }
         }
       //  cout<<endl;
