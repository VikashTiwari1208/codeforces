...
int32_t main()
{
    ...
    vvi dp(onsize+1,vi(zsize+1,INT_MAX));
    //dp[i][j] min sum to set i one using upto j zeroes
    ...
    for(int i=1;i<=onsize;i++)
    {
        for(int j=1;j<=zsize;j++)
        {
            ...
            if(i==j)
            {
              dp[i][j]=dp[i-1][j-1]+abs(one[i-1]-zero[j-1]);
            }
           else
           {
               
                int ans1=abs(one[i-1]-zero[j-1])+dp[i-1][j-1]; //give
                int ans2=dp[i][j-1]; // dont give
                dp[i][j]=min({dp[i][j],ans1,ans2});
           }
        }
    }
    cout<<dp[onsize][zsize];
    return 0;
}