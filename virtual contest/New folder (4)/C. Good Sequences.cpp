...
int32_t main()
{
    //we want gcd >1 so we kept max length of sequence for that gcd (ie factor) this is dp2
    // dp[i] is the maximum length of good sequence ending at number arr[i];
    //dp2[i] is the maximum length of good sequence for each x that is divisble by number arr[i]; so taht we can further use it
    fast;
     int n;
     cin>>n;
     vi arr(n+1);
    ...
    for(int i=1;i<=n;i++)
    {
        ...
        for(int j=2;j*j<=arr[i];j++)
        {
          if(arr[i]%j==0)
          {
              curr=max({curr,dp2[j],dp2[arr[i]/j]});
          }
        }
        dp[i]=curr+1;
        dp2[arr[i]]=max(dp2[arr[i]],dp[i]);
        for(int j=2;j*j<=arr[i];j++)
        {
            if(arr[i]%j==0)
            {
                dp2[j]=max(dp2[j],dp[i]);
                dp2[arr[i]/j]=max(dp2[arr[i]/j],dp[i]);
            }
        }
        ans=max(ans,dp[i]);
    }
