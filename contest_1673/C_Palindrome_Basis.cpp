...
vvi dp;
vi v;
int helper(int curr_sum,int idx)
{
    int m=v.size();
    if(curr_sum<0)
    {
   ...
   if(dp[curr_sum][idx]!=-1)
   {
       return dp[curr_sum][idx];
   }
   int ans=0;
   ans=(helper(curr_sum,idx+1)%M+helper(curr_sum-v[idx],idx)%M)%M;
   return dp[curr_sum][idx]=ans;

}
int32_t main()
{
    fast;
      dp.resize(maxn+3,vi(maxp,-1));
